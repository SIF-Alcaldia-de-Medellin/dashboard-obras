...
const Paneles = ({data}) => {
  ...
  const totalContratos = data.reduce((acc, obra) => {
    const contrato = obra.attributes.NUMERO_CON;
    acc.add(contrato); 
    return acc;
  }, new Set());

  const numeroDeContratos = totalContratos.size;
  ...
  return (
    <div className="flex justify-center gap-[10px] xl:gap-[20px] flex-wrap xl:flex-nowrap xl:justify-between">
      ...
      <Card className="bg-green-500 w-[100%] xl:w-1/5" icon={faBuildingCircleCheck} title="Terminadas" kpi={dataByEstado["Terminado"] || 0} />
      <Card className="bg-dark-blue-500 w-[100%] xl:w-1/5" icon={faBriefcase} title="Total Contratos" kpi={numeroDeContratos || 0} />
    </div>
  );
};
